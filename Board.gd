...
func move(san: String, is_recieved_move := true) -> void:
	if is_valid_move(san):
		...
		if move_0x88.flags & Chess.BITS.CAPTURE:
			...
			var _rook := get_piece(Chess.offset(move_0x88.to, Vector2(1, 0))).move(
				Chess.offset(move_0x88.to, Vector2(-1, 0))
			)
		...
		if move_0x88.flags & Chess.BITS.PROMOTION:  #promotion wow
			var p = board[move_0x88.from].move(Chess.algebraic(move_0x88.to))
			...
			make_piece(p.position, move_0x88.promotion, p.color)  # the move animation is useless if its not my turn
